...
- `Shift+Enter`: 改行
- 確定文字列の編集 (Emacs 風): `Ctrl+A` 行頭 / `Ctrl+E` 行末 / `Ctrl+F` `Ctrl+B` 1 文字前後 / `Ctrl+K` 行末まで削除 (行末では改行も削除して連結) / `Ctrl+U` 行頭まで削除 / `Ctrl+O` 全選択
- `Escape`: 変換中はキャンセル / 未変換状態なら窓を閉じる (コピーせず、入力内容は次回まで保持)
