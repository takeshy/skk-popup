...
- 登録モーダル内でも送り仮名が揃うと自動で候補変換する
- 登録モーダル内の `Ctrl+G`: 候補 → 変換中 → ローマ字 → モーダルを閉じる、と 1 段階ずつ取り消す
- 送り仮名あり変換の単語登録では読みを `はげ*る` のように表示 (送り仮名は確定時に自動付与され、辞書には語幹のみ登録)
...
- 送り仮名入力中 (候補未表示) の `Ctrl+G`: 送り仮名を読みへ畳み込み、1 つの見出しとして再変換できる状態に戻す
...
- `Shift+Enter`: 改行
- 確定文字列の編集 (Emacs 風): `Ctrl+A` 行頭 / `Ctrl+E` 行末 / `Ctrl+F` `Ctrl+B` 1 文字前後 / `Ctrl+K` 行末まで削除 (行末では改行も削除して連結) / `Ctrl+U` 行頭まで削除 / `Ctrl+O` 全選択 / `Ctrl+X` 選択を切り取り / `Ctrl+Z` 元に戻す
- `Escape` / `Ctrl+[`: 変換中はキャンセル / 未変換状態なら窓を閉じる (コピーせず、入力内容は次回まで保持)
